...
from scipy.stats import lognorm, genextreme, chi2
...
def annual_maximum(filename):
    ...
    annual_accum = data['Precipitação'].resample('Y').max()
...
station = "D:\Pesquisa\PIBIC 23-24\Estacoes\Estacoes_SIGA\DELMIRO_GOUVEIA.xlsx"
...
if chi2_logNormal > chi2_GEV:
    label_validate = 'A distribuição GEV representa melhor a distribuição empírica'
else:
    label_validate = 'A distribuição LogNormal representa melhor a distribuição empírica'
